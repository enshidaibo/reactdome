...
const UploadFilesBox = ({ type = 'audio', typeId, onOk, children }) => {
    const [visible, setVisible] = useState(false)
    const handleChangeVisible = (isVisible = true) => {
        setVisible(isVisible)
    }
    const uploadRequest = async (file, onProgress) => {
        return vmsUploadFile(file, { func_type: 2, typeId, onProgress });
    }
    return [
        ...
        <UploadBox key="uploadbox" show={visible} title={"音频上传"}>
            <Uploadfiles accept={type} uploadRequest={uploadRequest} >
                {(attachments, handleFileChange, handleRetry) => {
                    ...
                    <div className={styles.btns} key='btns'>
                        ...
                        <FilesButton accept={"audio/*"} onChange={handleFileChange} />
