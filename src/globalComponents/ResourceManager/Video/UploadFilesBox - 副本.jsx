...
export default class UploadFilesBox extends Component {
    ...
    render() {
        ...
        return [
            ...
            <UploadBox key="uploadbox" show={show} title={"视频上传"}>
                ...
                <div className={styles.btns}>
                    ...
                    <FilesButton accept={"video/*,video/x-matroska,video/x-flv"} onChange={this.handleFileChange} />
    ...
    handleFileUpload = async (file, index) => {
        ...
        let res = await vmsUploadFile(file, { func_type: 1, typeId, onProgress: res => this.onProgress(res, index) });
        if (res.success) {
            attachments[index].state = 3;
            attachments[index].path = res.data.videoUrl;
        } else {
