...
const UploadFilesBox = ({ type = 'image', typeId, mark = true, onOk, handleDirectInsertion, children, isDirect }) => {
    const [visible, setVisible] = useState(false)
    const handleChangeVisible = (isVisible = true) => {
        setVisible(isVisible)
    }
    const uploadRequest = async (file) => {
        return uploadFile(file, { typeId })
    }
    return [
        ...
        <UploadBox key="uploadbox" show={visible} title={"图片上传"}>
            <Uploadfiles accept="image/*" uploadRequest={uploadRequest} >
                {(attachments, handleFileChange, handleRetry) => {
                    ...
                    <div className={styles.btns} key='btns'>
                        ...
                        <FilesButton onChange={handleFileChange} />
                        <Button type="primary" className={styles.btn} onClick={handleRetry}>
                            重试上传失败文件
                    </Button>
